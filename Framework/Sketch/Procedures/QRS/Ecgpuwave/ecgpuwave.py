...
def lynfilt (x, fs, ns):
    ...
    Bd = np.array([1,2,0,-2,-1], dtype = 'float64')
    Bd = (fs/8) * Bd
    Ad = 1
    ...
    Bpa = np.zeros((1, mpaa))
    Bpa = np.asarray(Bpa).T
    Bpa[0] = 1
    Bpa[mpa] = -2
    Bpa[2*mpa] = 1
    # Apa = np.array([1,-2,1], dtype = 'float64')
    Apa = np.array([1,-2,1], dtype = 'int')
    #Apa = 1
...
def seek_pic1(x,thresh,number,sortem, *args):
...
def seek_pic2(x,thresh,number,sortem, *args):
...
def proces(X,Xpa,Xpb,D,F,Der,iprimerqrs,nqrs,iqrs,atyp,ns,Fs,nl,res,prewindt,Kq,Kr,Ks,Krr,Kpb,Kpe,Ktb,Kte,pco):
...
#% ---- T wave location and limits ----
    ...
    #% ---- P and T wave amplitudes ----
            ...
            if ipend[n-1]!=0: 
                ...
                POSPoffset[n-1]=ipend[n-1] 

            if iqbeg[n-1]!=0:

                POSQRSonset[n-1]=iqbeg[n-1]

            if iqrs[n-1]!=0:
                POSfiducial[n-1]=iqrs[n-1]
            ...
            if ir2pos[n-1]!=0:
                ...
                POSR2[n-1]=ir2pos[n-1]

            if isend[n-1]!=0:
                POSQRS[n-1]=iqrs[n-1]
                VALQRS[n-1]=isend[n-1]- iqbeg[n-1]
                POSQRSoffset[n-1]=isend[n-1]

            if itbeg[n-1]!=0: 

                POSTonset[n-1]=itbeg[n-1]

            if it2pos[n-1]!=0: 

                POST2[n-1]=it2pos[n-1]
            ...
            if itend[n-1]!=0: 
                prewindt=itend[n-1]
                POSToffset[n-1]=itend[n-1]
                

        n += 1
        
    QRS = np.array([POSQRSonset,POSQ,POSR,POSS,POSQRSoffset]).T
    Twav = np.array([POSTonset,POST,POST2,POSToffset]).T
    Pwav = np.array([POSPonset,POSP,POSPoffset]).T
