...
import numpy as np
# import WTdelineator
from .WT import WTdelineator
# from wt import waveletFilters
# from wt import waveletDecomp
# from wt import Rdetection
# from wt import QRSdelineation
# from wt import Tdelineation
# from wt import Pdelineation
...
def delineateMultiLeadECG(sig,fs):
    ...
    # #from WTdelineator.WTdelineator import as wt
    # import .WTdelineator as wt
    # from .WTdelineator import waveletFilters
    # from .WTdelineator import waveletDecomp
    # from .WTdelineator import Rdetection
    # from .WTdelineator import QRSdelineation
    # from .WTdelineator import Tdelineation
    # from .WTdelineator import Pdelineation
    ...
    while not last: # Continue delineating until the end of the signal
        ...
        WTfilters = WTdelineator.waveletFilters(N,fs) # Create the filters to apply the algorithme-a-trous
        w = []
        
        for lead in range(sig.shape[1]): # Find QRS complexes in each lead
            w += [WTdelineator.waveletDecomp(thisSig[:,lead], WTfilters)] # Save the decomposition of each lead
            Rb, nb = WTdelineator.Rdetection(w[-1],fs)
            R += [Rb]
            n += [nb]
        ...
        for lead in range(len(R)):            
            _, _, Onb, Endb = WTdelineator.QRSdelineation(R[lead],w[lead],n[lead],fs)
            T1,T2,TonB,TendB = WTdelineator.Tdelineation(Endb,w[lead],fs) # Detect and delineate the T wave
            P1,P2,PonB,PendB = WTdelineator.Pdelineation(Onb,w[lead],fs) # Detect and delineate the P wave
