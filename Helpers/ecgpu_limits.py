...
def limits (dbcode, patient_nr):
    ...
    if (dbcode=='qtdb2'):
        data = LoadFile(join(base_dir_raw, patient_nr), "pu1")
    else:
    ...
    #dim2 = round(dim)
    dim2 = int(dim)
    anottime = np.zeros(dim2)
    #anottime = np.zeros(nsamp, 1)
    pos = 0
    i = 0
    ext = ".atr"
    with open(join(base_dir_raw, patient_nr+ext), 'rb') as fid:
        #data_array = np.fromfile(fid, str, 2)
        #data_array.shape = (, 1)
        data_array = np.fromfile(fid, 'u1').reshape([-1, 2])
       ...
       # z= np.fromfile()
        ...
        #while not not fid:
            ...
            if A == 59:
                ...
                #skip.shape = (4,1)
                skip = np.fromfile(fid, 'u1').reshape([-1,4])
