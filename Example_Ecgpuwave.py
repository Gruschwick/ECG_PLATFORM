...
#WTDelin        
    ...
    # rpeaks = ecgpu_detector.XQRS(data.data[:,0],data.freq)[0]
    # rpeaks.detect()
    
    rpeaks = ecgpu_detector.xqrs_detect(sig=data.data[:,0], fs=data.freq)
    # rpeaks.detect()
    # XQRS = ecgpu_detector.XQRS(sig, fs)
    # rpeaks.detect()
    try: 
        ann_selector = [x in PhysionetConstants.all_beats for x in data.annotations.anntype]
        anntype_selected = np.array(data.annotations.anntype)[ann_selector]
    ...
    # rpeaks = ecgpu_detector.gqrs_detect(sig=data.data[:,0], fs=data.freq)
    # # rpeaks.detect()
