...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 18 00:12:01 2020
...
from Sketch.Procedures.QRS.Engz2 import EngzeeSegmenter2
...
def test_HamiltonSegmenter():
	E = EngzeeSegmenter2([])
	print(E.requiredDataFields())
	print(MITBIH.DataFields())
	print(E._canRunOn(MITBIH.DataFields()))
...
def test_Comparator():
    ...
    #on_HSpatients2 = [str(i) for i in list(range(1,5))]
    ...
    CMP.Databases[-1].addProcedure( EngzeeSegmenter2(on_HSpatients) )
    #CMP.Databases[-1].addProcedure( EngzeeSegmenter(on_HSpatients) )
    
    CMP.AddDatabase(CINC1(), "../Datasets/cinc1/data_raw")
    CMP.Databases[-1].addProcedure( EngzeeSegmenter2(on_HSpatients2) )
    #CMP.Databases[-1].addProcedure( EngzeeSegmenter(on_HSpatients2) )

    CMP.AddDatabase(CINC2(), "../Datasets/cinc2/data_raw")
    CMP.Databases[-1].addProcedure( EngzeeSegmenter2(on_HSpatients3) )
    #MP.Databases[-1].addProcedure( EngzeeSegmenter(on_HSpatients3) )
